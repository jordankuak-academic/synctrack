    ...
    <div class="project-body">
        <aside class="project-list-card">
            ...
            <div class="card-content">
                <div
                    ...
                    class="project-empty active{{ $projectCount > 0 ? ' has-projects' : '' }}">
                    ...
                    <div class="project-status-divider"></div>
        ...
        <section class="project-content-card">
            <div
                ...
                <div class="card-header">
                    <h2 class="heading-04">
                        {{ $projectCount === 0 ? 'No Project Available' : 'Select a Project' }}
                    </h2>
                </div>
                <div class="card-body empty-center">
                    <p class="body-l">
                        {{ $projectCount === 0 ? 'No project available.' : 'Select a project from the left to view tasks.' }}
                    </p>
                </div>
            </div>
            <div
                                ...
                                <div class="task-list" data-task-list>
                                    @forelse ($project['tasks'] ?? [] as $task)
                                        ...
                                        <div class="task-row" data-task-id="{{ $task['id'] }}" data-task-title="{{ $task['title'] }}" data-assignee-id="{{ $task['assignee_id'] ?? '' }}" data-assignee-name="{{ $assignee['username'] ?? '' }}" data-due-date="{{ !empty($task['due_date']) ? substr($task['due_date'], 0, 10) : '' }}" data-priority="{{ $taskPriority }}" data-status="{{ $taskStatus }}">
                                            ...
                                            <div class="date-column helper-text"><input class="task-inline-input task-date-input{{ !empty($task['due_date']) ? ' has-value' : '' }}" type="date" value="{{ !empty($task['due_date']) ? substr($task['due_date'], 0, 10) : '' }}" data-task-id="{{ $task['id'] }}" aria-label="Edit due date for {{ $task['title'] }}"></div>
                                            <div class="priority-column">
                                                ...
                                                @else
                                                    <select class="task-priority-select {{ $taskPriority ? strtolower($taskPriority) : 'default' }}" data-task-id="{{ $task['id'] }}" aria-label="Priority for {{ $task['title'] }}">
                                                        ...
                                                        @foreach (['Low', 'Medium', 'High'] as $priority)<option value="{{ $priority }}" @selected($taskPriority === $priority)>{{ $priority }}</option>@endforeach
                                                    </select>
                                                @endif
                                            </div>
                                            <div class="action-column"><button type="button" class="task-delete" data-task-id="{{ $task['id'] }}" aria-label="Delete {{ $task['title'] }}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M9 6V4h6v2m-8 0 1 14h8l1-14M10 10v6m4-6v6" /></svg></button></div>
                                        </div>
                                        @foreach ($subTasks as $subTask)
                                            ...
                                            <div class="task-row subtask-row" data-parent-task-id="{{ $task['id'] }}" data-task-id="{{ $subTask['id'] }}" data-task-title="{{ $subTask['title'] }}" data-assignee-id="{{ $subTask['assignee_id'] ?? '' }}" data-assignee-name="{{ $subTaskAssignee['username'] ?? '' }}" data-due-date="{{ !empty($subTask['due_date']) ? substr($subTask['due_date'], 0, 10) : '' }}" data-priority="{{ $subTaskPriority }}" data-status="{{ $subTaskStatus }}" hidden>
                                                <div class="status-column"><button type="button" class="task-status {{ $subTaskStatus }}" data-task-id="{{ $subTask['id'] }}" aria-label="{{ $subTask['title'] }}: {{ $subTaskStatusLabel }}">@if ($subTaskStatus === 'assigned')<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3h10M7 21h10M8 3c0 4 2 5 4 6-2 1-4 2-4 6M16 3c0 4-2 5-4 6 2 1 4 2 4 6" /></svg>@elseif ($subTaskStatus === 'done')<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 12 4 4 8-8" /></svg>@endif</button></div>
                                                <div class="task-name-column body-s"><input class="task-inline-input task-name-input" type="text" maxlength="100" value="{{ $subTask['title'] }}" placeholder="Task Name" data-task-id="{{ $subTask['id'] }}" aria-label="Edit task name"></div>
                                                <div class="assigned-column helper-text"><button type="button" class="member-picker" data-task-id="{{ $subTask['id'] }}" data-member-id="{{ $subTask['assignee_id'] ?? '' }}" data-value="{{ $subTaskAssignee['username'] ?? '' }}">{{ $subTaskAssignee['username'] ?? 'Member' }}</button></div>
                                                <div class="date-column helper-text"><input class="task-inline-input task-date-input{{ !empty($subTask['due_date']) ? ' has-value' : '' }}" type="date" value="{{ !empty($subTask['due_date']) ? substr($subTask['due_date'], 0, 10) : '' }}" data-task-id="{{ $subTask['id'] }}" aria-label="Edit due date for {{ $subTask['title'] }}"></div>
                                                <div class="priority-column"><select class="task-priority-select {{ $subTaskPriority ? strtolower($subTaskPriority) : 'default' }}" data-task-id="{{ $subTask['id'] }}" aria-label="Priority for {{ $subTask['title'] }}"><option value="" @selected($subTaskPriority === '')>Priority</option>@foreach (['Low', 'Medium', 'High'] as $priority)<option value="{{ $priority }}" @selected($subTaskPriority === $priority)>{{ $priority }}</option>@endforeach</select></div>
                                                <div class="action-column"><button type="button" class="task-delete" data-task-id="{{ $subTask['id'] }}" aria-label="Delete {{ $subTask['title'] }}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M9 6V4h6v2m-8 0 1 14h8l1-14M10 10v6m4-6v6" /></svg></button></div>
                                            </div>
                                        @endforeach
                                        <button type="button" class="create-subtask-btn hover-subtask-btn" data-parent-task-id="{{ $task['id'] }}"><span>+</span><span>Create SubTask</span></button>
                                    @empty
                ...
                <div id="project-detail-panels">
                    @foreach ($allProjects as $project)
                        <div class="project-detail-body project-detail-panel" data-project-id="{{ $project['id'] }}" hidden>
                            <div class="detail-field"><label class="label-m" for="detail-project-name-{{ $project['id'] }}">Project Name</label><input id="detail-project-name-{{ $project['id'] }}" data-detail-project-name type="text" maxlength="100" value="{{ $project['title'] }}" disabled></div>
                            <div class="detail-field"><label class="label-m" for="detail-project-description-{{ $project['id'] }}">Description</label><textarea id="detail-project-description-{{ $project['id'] }}" data-detail-project-description rows="5" disabled>{{ $project['description'] ?? '' }}</textarea></div>
                            <section class="detail-members">
                                <h3 class="label-m">Members</h3>
                                <div class="detail-member-table detail-member-head helper-text"><span>No</span><span>Name</span><span>Role</span><span>Email</span></div>
                                <div class="detail-member-list" data-detail-member-list>
                                    @forelse ($project['members'] ?? [] as $member)
                                        <div class="detail-member-row body-s" data-member-id="{{ $member['id'] }}" data-membership-id="{{ $member['membership_id'] ?? '' }}" data-member-name="{{ $member['username'] }}" data-member-role="{{ $member['role'] ?? (($member['is_owner'] ?? false) ? 'Owner' : 'Member') }}" data-member-email="{{ $member['email'] }}">
                                            <span>{{ $loop->iteration }}</span><span>{{ $member['username'] }}</span><span>{{ $member['role'] ?? (($member['is_owner'] ?? false) ? 'Owner' : 'Member') }}</span><span>{{ $member['email'] }}</span>
                                            @if (!empty($member['membership_id']))<button type="button" class="detail-member-delete" data-membership-id="{{ $member['membership_id'] }}" aria-label="Delete {{ $member['username'] }}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M9 6V4h6v2m-8 0 1 14h8l1-14M10 10v6m4-6v6" /></svg></button>@endif
                                        </div>
                                    @empty
                                        <div class="detail-member-empty helper-text">No members have been added.</div>
                                    @endforelse
                                </div>
                                <button type="button" class="detail-add-member body-s"><span aria-hidden="true">+</span><span>Add Other Member</span></button>
                            </section>
                        </div>
                    @endforeach
                </div>
            ...
            <div
                id="toast-project-update-success"
                class="project-toast success">
                <div class="toast-content">
                    <div class="toast-message helper-text">
                        Update project detail successfully.
                    </div>
                </div>
            </div>
            <div
                ...
                <div class="toast-content">
                    <div class="toast-message helper-text">
                        Update project detail failed. Please try again.
                    </div>
                </div>
            </div>
            <div
                id="toast-project-delete"
                class="project-toast success">
                <div class="toast-content">
                    <div class="toast-message helper-text">
            ...
            <div
                id="toast-member-add-error"
                class="project-toast error">
                <div class="toast-content">
                    <div class="toast-message helper-text">
                        Add member failed. Please try again.
                    </div>
                </div>
            </div>
            <div
                id="toast-member-delete"
                class="project-toast success">
                <div class="toast-content">
                    <div class="toast-message helper-text">
